...
Optimizer::Optimizer(map<int, string> nt, symbolTable* gt, vector<Quadruple> ic)
{
	name_table = nt;
	global_table = gt;
...
bool Optimizer::preOptimize()
{
	...
	{
		Quadruple* e = &intermediate_code[i];
		if (e->op == "jal")
		{
...
void Optimizer::partition()
{
	...
	for (auto i = 0; i < block_group.size(); i++)
	{
		...
		else//这个基本块不发生返回
		{
			...
			{
				if (label_loc.find(intermediate_code[prepos].res) != label_loc.end()\
					&& label_loc[intermediate_code[prepos].res] < pos)
				{
					pos = label_loc[intermediate_code[prepos].res];
					prepos = label_loc[intermediate_code[prepos].res];
				}
				prepos++;
			}
...
vector<DAGitem> Optimizer::geneDAG(int block_no)
{
	vector<DAGitem> DAG;
	...
	for (auto pos = block->begin; pos <= block->end; pos++)
	{
		...
		string A = intermediate_code[pos].res;
		int element_count;
...
void Optimizer::optimize()
{
	...
	for (int block_no = 0; block_no < block_group.size(); block_no++)
	{
		...
		for (auto i = newblock.begin; i < optimized_code.size(); i++)
		{
			...
			if (e.op == "+" && e.arg1 == "$sp" && is_num(e.arg2) && e.res == "$sp")
			{
				int sum = atoi(e.arg2.c_str());
				while (i + 1 < optimized_code.size() && optimized_code[i + 1].op == "+" && optimized_code[i + 1].arg1 == "$sp" && is_num(optimized_code[i + 1].arg2) && optimized_code[i + 1].res == "$sp")
				{
...
void Optimizer::showIntermediateCode()
{
	cout << "********Intermediate Code********\n";
	for (auto i = 0; i < intermediate_code.size(); i++)
	{
		cout << "(" << i << ")\t" << intermediate_code[i].op << '\t' << intermediate_code[i].arg1 << '\t' << intermediate_code[i].arg2 << '\t' << intermediate_code[i].res << endl;
	}
	cout << "*********************************\n";
}
void Optimizer::showBlockGroup()
{
	for (auto i = 0; i < block_group.size(); i++)
	{
